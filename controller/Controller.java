	...
	public void run() {
		...
		while (true) {
			if (model.canITakeFromServ()) {
				...
				if (servString.contains("winner.")) {
					winner();
					break;
				}
				if (servString.contains("loser.")) {
					loser();
					break;
				}
		...
		Platform.runLater(new Runnable() { 
			public void run() {
				for (int i = 0; i < 10; i++) {
					for (int j = 0; j < 10; j++) {
						if(cells[i][j] == 2) {
							getMyButton(i, j).setStyle("-fx-border-color: black");
							getMyButton(i, j).setStyle("-fx-color: green");
							getMyButton(i, j).setDisable(true);
						}
						else if(cells[i][j] == 4) {
							getMyButton(i, j).setStyle("-fx-color: red");
						}
						else if(cells[i][j] == 5) {
							getMyButton(i, j).setStyle("-fx-color: gray");
						...
						if(hisCells[i][j] == 1) {
							...
							getHisButton(i, j).setStyle("-fx-color: gray");
						}
						else if(hisCells[i][j] == 2) {
							getHisButton(i, j).setText("#");
							getHisButton(i, j).setStyle("-fx-color: red");
					...
					for (int j = 0; j < 10; j++) {
						if (i == 5) {
							if(j == 2) {
								...
								getMyButton(i, j).setText("");
								getHisButton(i, j).setText("");}
							getMyButton(i, j).setStyle("-fx-color: white");
							getHisButton(i, j).setText("-fx-color: gray");
						}
						else {
							getMyButton(i, j).setText("");
							getHisButton(i, j).setText("");
					...
					for (int j = 0; j < 10; j++) {
						if (i == 5) {
							if(j == 2) {
								...
								getHisButton(i, j).setText(" ");
								getMyButton(i, j).setText(" ");}
							getHisButton(i, j).setStyle("-fx-color: white");
							getMyButton(i, j).setStyle("-fx-color: gray");
						}
						else {
							getHisButton(i, j).setText(" ");
